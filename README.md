...
*   **動態色彩邏輯**：
    嚴格執行規格要求：**收盤價高於月平均價顯示紅字**，低於則顯示綠字。
*   **全域主題適配 (System-Wide Theme Adaptation)：**：
    利用 Android 資源目錄管理機制（values 與 values-night），針對背景色、文字顏色及元件色調進行深度優化。確保在不同亮度環境下，金融數據的視覺對比度均符合可讀性標準，減少使用者長時間盯盤的視覺疲勞。
...
- [x] **暗色模式支援**：通過values和values-night不同的colors資源檔，來實現完整的暗色模式。
---
...
*   **核心模組化設計**：
    ...
    *   **`:AssetsLibs`**：封裝取得Assets的非同步方法工具庫
---
